...
class Config:
    def __init__(self, freq, channels,seq_len,pred_len,d_model):
        self.freq = freq
...
if __name__ == '__main__':
    configs = Config('h', 7,96,96,512)
    model = FBNet.Model(configs)
    # 加载到cpu
    root_path = '../dataset/'
    file_name = 'etth2'

    model.load_state_dict(torch.load('../show/checkpoints/'  +file_name +'_96'+ '.pth', map_location=torch.device('cpu')))
    model.eval()
    #获取权重
    print(model)
